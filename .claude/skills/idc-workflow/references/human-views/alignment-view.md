...
Approval 请求必须通过 `AskUserTool` 发出；本文件只定义用户可读展示，不是直接 approval 通道。
...
- approve / request clarification / request reclassify / reject 必须通过 `AskUserTool` 收集；如果不可用，返回 `BLOCKED_NEEDS_ASK_USER_TOOL`。
