...
| 输出件 | 规则出处 |
|---|---|
| `effective-team-config.yaml`（`.idc/`，框架生成） | `scripts/prepare_runtime.rb`；`idc-workflow/SKILL.md` Required behavior |
...
| 输出件 | 规则出处 |
|---|---|
| **计划件本体（硬性，新增钉死）**：`general-plan.yaml` 或 `d3a-plan.yaml`。计划件必须落盘并符合对应 schema；`execution_authorization_request.technical_plan_confirmation.confirmation_ref` 必须指向这个文件，`authorize_execution.rb` 校验其存在——文件缺失即 `BLOCKED_PLAN_CONFIRMATION_REQUIRED`，这就是强制点。 | `schemas/general-plan.schema.yaml`；`schemas/d3a-plan.schema.yaml`；`workflows/execution-authorization-gate.md` |
...
| capability plan（READY） | `schemas/capability-selection.schema.yaml`；`scripts/select_capabilities.rb` |
| knowledge plan（READY，绑定 execution unit） | `schemas/knowledge-load-plan.schema.yaml`；`scripts/plan_knowledge.rb` |
...
| execution-authorization request + result（含 `technical_plan_confirmation` echo） | `schemas/execution-authorization.schema.yaml`；`scripts/authorize_execution.rb` |
...
| knowledge consumption receipt（completion 前 VERIFIED） | `scripts/verify_knowledge_consumption.rb` |

## completion

| 输出件 | 规则出处 |
|---|---|
| completion result / summary（`DONE` 只能来自工具判定） | `scripts/verify_completion.rb`；`schemas/completion-verification.schema.yaml` |
