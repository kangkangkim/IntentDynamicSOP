...
12. 所有用户问题、确认、approval、re-alignment 和 escalation 决策，优先使用宿主结构化确认工具。Claude Code和CodeAgent 使用 AskUserQuestion；Codex Plan 模式使用 request_user_input；Codex Default 模式无结构化工具时，允许用户明确文本批准作为等价确认，但必须落盘 approval record，禁止模型自我确认。
